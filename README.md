...
**VTT is a multiplayer environment for you and your firends to host and play tabletop roleplaying games. Create magnificent 3D environments or more traditional 2D ones.**
...
* A very rich property editor for both objects and maps allows for fine control over any value or property to create truly any environment imagineable. Many of the finer controls have tooltips, explaining exactly what a given property controls in great detail.
* A dynamic fog of war system for both 3D and 2D environments, with all the scaleability and fine controls you'd ever need.
* Easy to setup multiplayer - just click host, enter the desired port and let your players connect.
* An anvanced distance measuring system that allows you to measure lines, circles, spheres, squares, cubes and cones, highlighting which objects fall into the measured shape, with the ability to leave those measurments in the scene with custom tooltips.
...
* Custom health/mana/armor/anything bars for objects, with automatic value calculation for mathematical inputs for ease of use.
...
## Troubleshooting
**Application doesn't startup**: Make sure that you have the required components installed and they match the arcgitecture of your os.
...
**Application crashes**: Whenever VTT encounters an unrecoverable problem it will shutdown, generating a crashreport file in the main application directory. Please submit a bugreport in the issues section here, including this file and other relevant information (such as log files).
...
* If your modeling software allows, it is recommended to export tangents and bitangents with the model, as automatically generated ones may differ from those of the software and cause rendering differences.
...
* All point and directional lights will be exported with the model, and this is in fact how you create lights in your scenes. However due to rendering differences (notably the lack of reflections) the lights may appear dimmer in your modeling editor than they are in VTT.
...
* Download full FFmpeg binaries. The ones that include both the .exe and .dll files are required (specifically, the following files must be present:)
   * avcodec-**.dll
   * avdevice-**.dll
   * avfilter-*.dll
   * avformat-**.dll
...
You can edit, move and delete files. An asset file can't be loaded if either .ab or .json are not present, however it is recommended that you delete both if you want to delete either. Please note that .ab is not necessairly the .glb or .png that you've uploaded, and your modeling software may be unable to open them.
Never edit these files while the application is running.
Please do not edit the map fog of war .png files manualy. They may be stored as .png but they don't contain easily editable data for any image editing software.
...
In case you don't wich to use Visual Studio the following nuget packages are used:
* CoreCLR-NCalc
* FFmpeg.AutoGen
* glTF2Loader
* ImGui .NET
...
**-connect [ip:port]**: Launches VTT and tries to connect to the adress specified, bypassing the main menu.
