...
r = $INPUT@6$;
",
(matrix, outIndex) => new NodeSimulationMatrix(matrix[0])
        );
...
@"$OUTPUT@0$ = $INPUT@0$ * $INPUT@1$;",
        (matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (Vector3)x[0] * (Vector3)x[1])
        );
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Vector3.Dot((Vector3)x[0], (Vector3)x[1]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Vector3.Cross((Vector3)x[0], (Vector3)x[1]))
);
...
@"$OUTPUT@0$ = -$INPUT@0$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => -(Vector3)x[0])
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Normalized())
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Abs())
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Ceil())
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Floor())
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Round()));
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Clamp((Vector3)x[1], (Vector3)x[2]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Min((Vector3)x[1]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Max((Vector3)x[1]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Vector3.Lerp((Vector3)x[0], (Vector3)x[1], (float)x[2]))
);
...
@"$OUTPUT@0$ = $INPUT@0$ * $INPUT@1$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (Vector2)x[0] * (Vector2)x[1])
        );
...
@"$OUTPUT@0$ = dot($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Vector2.Dot((Vector2)x[0], (Vector2)x[1]))
);
...
@"$OUTPUT@0$ = -$INPUT@0$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => -(Vector2)x[0])
);
...
@"$OUTPUT@0$ = normalize($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Normalized())
);
...
@"$OUTPUT@0$ = abs($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Abs())
);
...
@"$OUTPUT@0$ = ceil($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Ceil())
);
...
@"$OUTPUT@0$ = floor($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Floor())
);
...
@"$OUTPUT@0$ = round($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Round())
);
...
@"$OUTPUT@0$ = clamp($INPUT@0$, $INPUT@1$, $INPUT@2$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Clamp((Vector2)x[1], (Vector2)x[2]))
);
...
@"$OUTPUT@0$ = min($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Min((Vector2)x[1]))
);
...
@"$OUTPUT@0$ = max($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Max((Vector2)x[1]))
);
...
@"$OUTPUT@0$ = mix($INPUT@0$, $INPUT@1$, $INPUT@2$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Vector2.Lerp((Vector2)x[0], (Vector2)x[1], (float)x[2]))
);
...
@"$OUTPUT@0$ = $INPUT@0$ * $INPUT@1$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (Vector4)x[0] * (Vector4)x[1])
        );
...
@"$OUTPUT@0$ = dot($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Vector4.Dot((Vector4)x[0], (Vector4)x[1]))
);
...
@"$OUTPUT@0$ = -$INPUT@0$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => -(Vector4)x[0])
);
...
@"$OUTPUT@0$ = normalize($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Normalized())
);
...
@"$OUTPUT@0$ = abs($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Abs())
);
...
@"$OUTPUT@0$ = ceil($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Ceil())
);
...
@"$OUTPUT@0$ = floor($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Floor())
);
...
@"$OUTPUT@0$ = round($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Round())
);
...
@"$OUTPUT@0$ = clamp($INPUT@0$, $INPUT@1$, $INPUT@2$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Clamp((Vector4)x[1], (Vector4)x[2]))
);
...
@"$OUTPUT@0$ = min($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Min((Vector4)x[1]))
);
...
@"$OUTPUT@0$ = max($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Max((Vector4)x[1]))
);
...
@"$OUTPUT@0$ = mix($INPUT@0$, $INPUT@1$, $INPUT@2$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Vector4.Lerp((Vector4)x[0], (Vector4)x[1], (float)x[2]))
);
...
@"$OUTPUT@0$ = $INPUT@0$ * $INPUT@1$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (float)x[0] * (float)x[1])
);
...
@"$OUTPUT@0$ = abs($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Abs((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Sin((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Cos((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Asin((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Acos((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Tan((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Atan((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Atan2((float)x[0], (float)x[1]))
);
...
@"$OUTPUT@0$ = ceil($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Ceiling((float)x[0]))
);
...
@"$OUTPUT@0$ = floor($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Floor((float)x[0]))
);
...
@"$OUTPUT@0$ = round($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Round((float)x[0]))
);
...
@"$OUTPUT@0$ = clamp($INPUT@0$, $INPUT@1$, $INPUT@2$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Math.Clamp((float)x[0], (float)x[1], (float)x[2]))
);
...
@"$OUTPUT@0$ = min($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Min((float)x[0], (float)x[1]))
);
...
@"$OUTPUT@0$ = max($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Max((float)x[0], (float)x[1]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (float)x[0] * 180 / MathF.PI)
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (float)x[0] * MathF.PI / 180)
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (float)x[0]) // no derivative calculation possible?
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Exp((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Pow(2.0f, (float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (float)((decimal)(float)x[0] % 1.0m))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Log((float)x[0]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Log2((float)x[0]))
);
...
@"$OUTPUT@0$ = mix($INPUT@0$, $INPUT@1$, $INPUT@2$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((float)x[0] * (1.0f - (float)x[2])) + ((float)x[1] * (float)x[2]))
);
...
@"$OUTPUT@0$ = pow($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Pow((float)x[0], (float)x[1]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Sqrt((float)x[0]))
);
...
@"$OUTPUT@0$ = abs($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Math.Abs((int)x[0]))
);
...
@"$OUTPUT@0$ = clamp($INPUT@0$, $INPUT@1$, $INPUT@2$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Math.Clamp((int)x[0], (int)x[1], (int)x[2]))
);
...
@"$OUTPUT@0$ = min($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Math.Min((int)x[0], (int)x[2]))
);
...
@"$OUTPUT@0$ = max($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => Math.Max((int)x[0], (int)x[2]))
);
...
@"$OUTPUT@0$ = $INPUT@0$ * $INPUT@1$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (int)x[0] * (int)x[1])
);
...
@"$OUTPUT@0$ = pow($INPUT@0$, $INPUT@1$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (int)Math.Pow((int)x[0], (int)x[1]))
);
...
@"$OUTPUT@0$ = sqrt($INPUT@0$);",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (int)Math.Sqrt((int)x[0]))
);
...
(matrix, outIndex) => outIndex switch {
    ...
    6 => new NodeSimulationMatrix(dummyAOMR.Value).Cast<Vector4, float>(x => x.Z),
    7 => new NodeSimulationMatrix(Vector3.One),
    8 => new NodeSimulationMatrix(1.0f),
    9 => new NodeSimulationMatrix(1.0f),
    10 => new NodeSimulationMatrix(1.0f),
    11 => new NodeSimulationMatrix(0u),
    _ => new NodeSimulationMatrix(0)
});
...
@"$OUTPUT@0$ = alpha;",
(matrix, outIndex) => new NodeSimulationMatrix(1.0f)
);
...
(matrix, outIndex) => outIndex switch {
    0 => new NodeSimulationMatrix(Vector3.One),
...
$OUTPUT@1$ = update;",
(matrix, outIndex) => outIndex switch {
    0 => new NodeSimulationMatrix((uint)Client.Instance.Frontend.FramesExisted),
    _ => new NodeSimulationMatrix((uint)Client.Instance.Frontend.UpdatesExisted)
}
);
...
(matrix, outIndex) => outIndex switch {
    ...
    2 => new NodeSimulationMatrix(new Vector3(0, 0, 1)),
    3 => new NodeSimulationMatrix(new Vector3(0, 0, 0)),
    4 => new NodeSimulationMatrix(new Vector3(1, 0, 0)),
    5 => new NodeSimulationMatrix(new Vector3(0, 1, 0)),
...
@"$OUTPUT@0$ = f_texture;",
(matrix, outIndex) => NodeSimulationMatrix.SimulateUVMatrix()
);
...
(matrix, outIndex) => outIndex switch { 
    0 => new NodeSimulationMatrix(Vector3.Zero),
    _ => new NodeSimulationMatrix(Vector2.Zero),
});
...
(matrix, outIndex) => outIndex switch {
    0 => new NodeSimulationMatrix(0),
    1 => new NodeSimulationMatrix(0),
    2 => new NodeSimulationMatrix(Vector4.One),
    _ => new NodeSimulationMatrix(1.0f - (Client.Instance.Frontend.UpdatesExisted % 60 / 60.0f)),
});
...
$OUTPUT@2$ = $TEMP@0$.b;",
(matrix, outIndex) => outIndex switch {
    0 => new NodeSimulationMatrix(dummyAOMR.Value, matrix[0]).Cast<Vector4, float>(x => x.X),
    1 => new NodeSimulationMatrix(0.0f),
    _ => new NodeSimulationMatrix(dummyAOMR.Value, matrix[0]).Cast<Vector4, float>(x => x.Z)
});
...
$OUTPUT@1$ = $INPUT@0$.y;",
(matrix, outIndex) => outIndex switch { 
    0 => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).X),
    _ => NodeSimulationMatrix.Parallel(matrix, x => ((Vector2)x[0]).Y),
});
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => new Vector2((float)x[0], (float)x[1]))
);
...
$OUTPUT@2$ = $INPUT@0$.z;",
(matrix, outIndex) => outIndex switch {
    0 => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).X),
    1 => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Y),
    _ => NodeSimulationMatrix.Parallel(matrix, x => ((Vector3)x[0]).Z),
});
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => new Vector3((float)x[0], (float)x[1], (float)x[2]))
);
...
$OUTPUT@3$ = $INPUT@0$.w;",
(matrix, outIndex) => outIndex switch {
    0 => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).X),
    1 => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Y),
    2 => NodeSimulationMatrix.Parallel(matrix, x => ((Vector4)x[0]).Z),
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => new Vector4((float)x[0], (float)x[1], (float)x[2], (float)x[3]))
);
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (bool)x[3] ? (int)x[0] >= (int)x[1] && (int)x[0] <= (int)x[2] : (int)x[0] > (int)x[1] && (int)x[0] < (int)x[2])
        );
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Abs((float)x[0] - (float)x[1]) <= 0.001f)
        );
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => MathF.Abs((float)x[0] - (float)x[1]) <= (float)x[2])
        );
...
@"$OUTPUT@0$ = $INPUT@3$ ? $INPUT@0$ >= $INPUT@1$ && $INPUT@0$ <= $INPUT@2$ : $INPUT@0$ > $INPUT@1$ && $INPUT@0$ < $INPUT@2$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (bool)x[3] ? (float)x[0] >= (float)x[1] && (float)x[0] <= (float)x[2] : (float)x[0] > (float)x[1] && (float)x[0] < (float)x[2])
        );
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (bool)x[0] ? 1 : 0)
        );
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (bool)x[0] ? 1f : 0f)
        );
...
@"$OUTPUT@0$ = !$INPUT@0$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => !(bool)x[0])
        );
...
@"$OUTPUT@0$ = $INPUT@0$ && $INPUT@1$;",
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => (bool)x[0] && (bool)x[1])
        );
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => ((bool)x[0] || (bool)x[1]) && !((bool)x[0] && (bool)x[1]))
        );
...
(matrix, outIndex) => NodeSimulationMatrix.Parallel(matrix, x => !((bool)x[0] && (bool)x[1]))
        ...
        public ShaderNodeTemplate(Guid id, ShaderTemplateCategory cat, string v, bool deletable, NodeInput[] nodeInputs, NodeOutput[] nodeOutputs, string code, Func<NodeSimulationMatrix[], int, NodeSimulationMatrix> sim)
        {
            this.Name = v;
        ...
        public Func<NodeSimulationMatrix[], int, NodeSimulationMatrix> Simulator { get; } 
